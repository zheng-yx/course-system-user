<template>
    ...
    <div>
        <template v-if="this.$route.params.id === undefined">
            <div style="width: 100%;">
                <!-- 标题、时间 -->
                <router-link :to="{path: '/achievementData/' + value.id}" class="fontStyle news_title" v-for="(value, i) in list" :key="i">
    ...
    fetch(params = {}) {
      ...
      if(this.$route.params.id === undefined) {
        ...
        for(var i = 0; i < this.list.length; i++) {
          ...
          if(this.list[i].createTime !== '' || this.list[i].createTime !== null) {
            this.list[i].createTime = this.list[i].createTime.substring(0, 10)
          }
        }
        this.loading = false
        })
